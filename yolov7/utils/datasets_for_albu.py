...
class LoadImagesAndLabels(Dataset):  # for training/testing
        ...
        if cache_images:
            ...
            for i, x in pbar:
                if cache_images == "disk":
                    ...
                    self.imgs[i], self.img_hw0[i], self.img_hw[i], self.imgs0[i] = x
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            # Load image
            img, (h0, w0), (h, w), img0 = load_image(self, index)
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        return (
            ...
            img.shape[:2],
            img0,
        )  # img, hw_original, hw_resized, original_img
    else:
        return (
            self.imgs[index],
            self.img_hw0[index],
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        # Load image
        img, _, (h, w), img0 = load_image(self, index)
        # print(img0.shape)
        ...
        if labels.shape == np.array([]).shape:
            labels = np.zeros((0, 5))
